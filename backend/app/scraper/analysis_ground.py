"""Scrapes PlayerGrounds*.asp → stats_by_ground table."""
...
FORMAT_PATHS = {
    "test": "PlayerGrounds.asp",
    "odi": "PlayerGrounds_ODI.asp",
    "t20i": "PlayerGrounds_T20.asp",
}


async def scrape_analysis_ground(player_db_id: str, howstat_id: int) -> None:
    ...
    for fmt, path in FORMAT_PATHS.items():
        try:
            ...
            for table in soup.find_all("table"):
                ...
                if not headers or not any(h in ("ground", "venue", "stadium") for h in headers):
                    continue
                ...
                for tr in table.find_all("tr")[1:]:
                    ...
                    ground = _clean(g("ground", "venue", "stadium"))
                    if not ground:
                        continue
                ...
                if clean_rows:
                    supabase.table("stats_by_ground").upsert(
                        clean_rows, on_conflict="player_id,format,ground"
                    ).execute()
                break

        except Exception as exc:
            print(f"[analysis_ground] Failed {fmt} for player {howstat_id}: {exc}")

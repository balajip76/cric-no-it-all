"""Scrapes PlayerYears*.asp → stats_by_year table."""
...
FORMAT_PATHS = {
    "test": "PlayerYears.asp",
    "odi": "PlayerYears_ODI.asp",
    "t20i": "PlayerYears_T20.asp",
}


async def scrape_analysis_year(player_db_id: str, howstat_id: int) -> None:
    ...
    for fmt, path in FORMAT_PATHS.items():
        try:
            ...
            for table in soup.find_all("table"):
                ...
                if not headers or "year" not in " ".join(headers):
                    continue
                ...
                if clean_rows:
                    supabase.table("stats_by_year").upsert(
                        clean_rows, on_conflict="player_id,format,year"
                    ).execute()
                break  # only need first matching table

        except Exception as exc:
            print(f"[analysis_year] Failed {fmt} for player {howstat_id}: {exc}")
